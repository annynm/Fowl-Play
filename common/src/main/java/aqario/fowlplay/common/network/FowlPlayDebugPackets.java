...
public class FowlPlayDebugPackets {
  ...
  public static <T> void sendGenericData(LivingEntity entity, Pair<String, T>... data) {
    ...
    GenericDebugPayload.Data payloadData =
        new GenericDebugPayload.Data(entity.getUUID(), entity.getId(), entity.position(), map);
  ...
  private static List<String> getMemoryDescriptions(LivingEntity entity, long gameTime) {
    Map<MemoryModuleType<?>, Optional<? extends ExpirableValue<?>>> map =
        entity.getBrain().getMemories();
    List<String> list = Lists.newArrayList();
    for (Map.Entry<MemoryModuleType<?>, Optional<? extends ExpirableValue<?>>> entry :
        map.entrySet()) {
