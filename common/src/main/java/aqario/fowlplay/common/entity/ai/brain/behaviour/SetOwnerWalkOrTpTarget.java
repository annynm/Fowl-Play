...
public class SetOwnerWalkOrTpTarget extends SpeedModifiableBehaviour<PigeonEntity> {
  ...
  protected boolean checkExtraStartConditions(ServerLevel world, PigeonEntity pigeon) {
    ...
    // Try to get owner via UUID first, then fallback to getOwner()
    UUID ownerUuid = pigeon.getOwnerUUID();
    if (ownerUuid != null) {
      owner = world.getPlayerByUUID(ownerUuid);
    }
    if (owner == null) {
      owner = pigeon.getOwner();
    }
    if (owner == null) {
      return false;
    }
    ...
    if (--this.updateCountdownTicks <= 0) {
      ...
      if (!pigeon.isLeashed() && !pigeon.isPassenger()) {
        if (pigeon.distanceToSqr(this.owner) >= 144.0) {
          pigeon.setMemory(FPMemoryTypes.TELEPORT_TARGET.get(), new TeleportTarget(this.owner));
        }
        pigeon.setMemory(
            MemoryModuleType.WALK_TARGET,
            new WalkTarget(this.owner, this.speedModifier.apply(pigeon, this.owner.position()), 0));
      }
    }
  }
}