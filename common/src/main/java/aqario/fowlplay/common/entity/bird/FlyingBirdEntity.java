...
public abstract class FlyingBirdEntity extends BirdEntity {
  ...
  public void loadCustomDataFromTag(
      CompoundTag nbt, net.minecraft.core.RegistryAccess registryAccess) {
    this.setFlying(nbt.getBoolean(FLYING_KEY).orElse(false));
  }
  ...
  public void tick() {
    ...
    if (!this.level().isClientSide()) {
      if (this.isFlying()) {
        ...
        if (this.shouldStopFlying()) this.stopFlying();
      } else {
        this.timeFlying = 0;
        this.setNoGravity(false);
      }
      if (this.isFlying() != this.isFlightNavigation) this.setNavigation(this.isFlying());
    }
  ...
  protected void updateAnimationStates() {
    ...
    if (!this.isFlying() && !this.isUnderWater()) {
    ...
    this.swimmingState.animateWhen(this.isUnderWater() && !this.isFlying(), this.tickCount);
  }
  ...
  protected void checkFallDamage(
      double heightDifference, boolean onGround, BlockState landedState, BlockPos landedPosition) {
    if (!this.isFlying())
  ...
  public boolean shouldStopFlying() {
    if (this.isFlightless() || this.isUnderWater() || this.isPassenger()) return true;
    if (this.timeFlying < MIN_FLIGHT_TICKS) return false;
  ...
  public boolean isFlying() {
    return !this.isFlightless() && this.entityData.get(FLYING);
  }
  ...
  public void calculateEntityAnimation(boolean flutter) {
    ...
    this.walkAnimation.update(targetSpeed, 0.4F, this.tickCount);
    this.prevHorizontalVelocity = horizontalVelocity;
  }

  private static float getFlapSpeed(
      float horizontalVelocity, float horizontalAcceleration, float verticalVelocity) {
    float kHV = 1.5F, kHA = 2.0F, kVV = 3.0F;
    float inverseHorizontal = Mth.clamp(1.0F - horizontalVelocity, 0.0F, 1.0F);
    float horizontalAccel = Math.abs(horizontalAcceleration);
    float vertical = Math.max(verticalVelocity, 0.0F);
