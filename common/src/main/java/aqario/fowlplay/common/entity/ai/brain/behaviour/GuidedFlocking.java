...
public class GuidedFlocking extends ExtendedBehaviour<FlyingBirdEntity> {
  private static final MemoryList MEMORIES =
      MemoryList.create(3)
          ...
          .absent(FPMemoryTypes.IS_AVOIDING.get(), FPMemoryTypes.SEES_FOOD.get());
  ...
  protected boolean checkExtraStartConditions(ServerLevel world, FlyingBirdEntity bird) {
    ...
    // Fixed: Replaced BrainUtils.hasMemory with direct brain access
    if (!brain.hasMemoryValue(FPMemoryTypes.NEAREST_VISIBLE_ADULTS.get())) {
      return false;
    }
    // Fixed: Replaced BrainUtils.getMemory with direct brain access
    this.nearbyBirds = brain.getMemory(FPMemoryTypes.NEAREST_VISIBLE_ADULTS.get()).orElse(null);
    if (this.nearbyBirds == null) {
      return false;
    }
  ...
  protected void tick(FlyingBirdEntity bird) {
    ...
    bird.getMoveControl()
        .setWantedPosition(
            ...
            heading.y,
            heading.z,
            (bird.getRandom().nextFloat() - bird.getRandom().nextFloat()) * 1.5 + 2);
  }
