...
public abstract class BirdEntity extends Animal {
  ...
  protected BirdEntity(EntityType<? extends BirdEntity> entityType, Level world) {
    ...
    this.idleAnimationChance =
        this.random.nextInt(this.getIdleAnimationDelay()) - this.getIdleAnimationDelay();
  ...
  public void addAdditionalSaveData(CompoundTag nbt) {
    ...
    if (this.isMemoryPresent(MemoryModuleType.HAS_HUNTING_COOLDOWN)) {
      nbt.putLong(
          HUNTING_COOLDOWN_KEY, this.getMemoryExpiry(MemoryModuleType.HAS_HUNTING_COOLDOWN));
    }
  }
  ...
  public boolean isWaterAboveFloatHeight() {
    return this.isUnderWater()
        || this.getFluidHeight(FluidTags.WATER) > this.getBoundingBox().getYsize() * 0.35;
  }
  ...
  @Override
  public Vec3 getFluidFallingAdjustedMovement(
      double gravity, boolean isFalling, Vec3 deltaMovement) {
  ...
  protected void resetIdleAnimationDelay() {
    this.idleAnimationChance =
        -(this.getIdleAnimationDelay() + this.random.nextIntBetweenInclusive(-200, 200));
  }
