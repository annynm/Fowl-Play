...
public record GullVariant(Identifier texture) {
  public static final StreamCodec<RegistryFriendlyByteBuf, Holder<GullVariant>> PACKET_CODEC =
      ByteBufCodecs.holderRegistry(FPRegistries.GULL_VARIANT);

  public static final CommonRegister<GullVariant> REGISTRAR =
      CommonRegister.create(FPRegistries.GULL_VARIANT, FowlPlay.ID);
  ...
  private static ResourceKey<GullVariant> register(String id) {
    ...
    Identifier texture =
        FowlPlay.id("textures/entity/gull/" + key.identifier().getPath() + "_gull.png");
