...
public class SetBreatheTarget<E extends BirdEntity> extends SpeedModifiableBehaviour<E> {
  private static final MemoryList MEMORIES =
      MemoryList.create(1).registered(MemoryModuleType.WALK_TARGET);
  ...
  protected void tick(E bird) {
    ...
    brain.setMemory(
        ...
        new WalkTarget(targetPos, this.speedModifier.apply(bird, targetPos), 0));
  }
  ...
  private boolean isAirPos(LevelReader world, BlockPos pos) {
    ...
    return (world.getFluidState(pos).isEmpty() || blockState.is(Blocks.BUBBLE_COLUMN))
