...
public record PigeonVariant(Identifier texture) {
  public static final StreamCodec<RegistryFriendlyByteBuf, Holder<PigeonVariant>> PACKET_CODEC =
      ByteBufCodecs.holderRegistry(FPRegistries.PIGEON_VARIANT);

  public static final CommonRegister<PigeonVariant> REGISTRAR =
      CommonRegister.create(FPRegistries.PIGEON_VARIANT, FowlPlay.ID);
  ...
  private static ResourceKey<PigeonVariant> register(String id) {
    ResourceKey<PigeonVariant> key =
        ResourceKey.create(FPRegistries.PIGEON_VARIANT, FowlPlay.id(id));
    // Fixed: ResourceKey.location() removed in 1.21.2+. Use key.location() instead.
    Identifier texture =
        FowlPlay.id("textures/entity/pigeon/" + key.identifier().getPath() + "_pigeon.png");
