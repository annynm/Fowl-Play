...
public record ChickenVariant(String id) {
  public static final StreamCodec<RegistryFriendlyByteBuf, Holder<ChickenVariant>> PACKET_CODEC =
      ByteBufCodecs.holderRegistry(FPRegistries.CHICKEN_VARIANT);
  public static final CommonRegister<ChickenVariant> REGISTRAR =
      CommonRegister.create(FPRegistries.CHICKEN_VARIANT, FowlPlay.ID);
  ...
  private static ResourceKey<ChickenVariant> register(String id) {
    ResourceKey<ChickenVariant> key =
        ResourceKey.create(FPRegistries.CHICKEN_VARIANT, FowlPlay.id(id));
