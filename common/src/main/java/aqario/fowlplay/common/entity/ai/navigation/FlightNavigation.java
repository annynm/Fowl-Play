...
public class FlightNavigation extends GroundPathNavigation implements ExtendedNavigator {
  ...
  public Path patchPath(@Nullable Path path) {
    ...
    if (debugNodeInfo != null) {
      newPath.setDebug(
          debugNodeInfo.openSet(), debugNodeInfo.closedSet(), debugNodeInfo.targetNodes());
    }
    return newPath;
  }
  ...
  protected void followThePath() {
    ...
    this.maxDistanceToWaypoint =
        this.bird.getBbWidth() > 0.75f
            ? this.bird.getBbWidth() / 2f
            : 0.75f - this.bird.getBbWidth() / 2f;
    ...
    if (!this.attemptShortcut(shortcutNodeIndex, pos)) {
      if (this.isCloseToNextNode(NODE_REACH_RADIUS)) {
        ...
        if (this.path.getNextNodeIndex() < this.path.getNodeCount() - 1
            && nextNodeIndex >= this.path.getNodeCount()) {
