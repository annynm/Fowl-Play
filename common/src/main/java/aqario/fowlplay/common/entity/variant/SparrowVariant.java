...
public record SparrowVariant(Identifier texture) {
  public static final StreamCodec<RegistryFriendlyByteBuf, Holder<SparrowVariant>> PACKET_CODEC =
      ByteBufCodecs.holderRegistry(FPRegistries.SPARROW_VARIANT);

  public static final CommonRegister<SparrowVariant> REGISTRAR =
      CommonRegister.create(FPRegistries.SPARROW_VARIANT, FowlPlay.ID);
  ...
  private static ResourceKey<SparrowVariant> register(String id) {
    ResourceKey<SparrowVariant> key =
        ResourceKey.create(FPRegistries.SPARROW_VARIANT, FowlPlay.id(id));
    // Fixed: ResourceKey.location() removed in 1.21.2+. Use key.location() instead.
    Identifier texture =
        FowlPlay.id("textures/entity/sparrow/" + key.identifier().getPath() + "_sparrow.png");
