...
public class RavenEntity extends TrustingBirdEntity implements BirdBrain<RavenEntity> {
  ...
  public Ingredient getFood() {
    return Ingredient.of(net.minecraft.core.registries.BuiltInRegistries.ITEM.getOrThrow(FowlPlayItemTags.RAVEN_FOOD));
  }
  ...
  public boolean shouldAttack(LivingEntity target) {
    ...
    LivingEntity hurtBy = this.brain.getMemory(MemoryModuleType.HURT_BY_ENTITY).orElse(null);
    if (!target.getType().is(FowlPlayEntityTypeTags.RAVEN_ATTACK_TARGETS)
        && (hurtBy == null || !hurtBy.equals(target))) return false;
    Optional<List<? extends AgeableMob>> nearbyAdults =
        this.brain.getMemory(FPMemoryTypes.NEAREST_VISIBLE_ADULTS.get());
  ...
  public BrainActivityGroup<? extends RavenEntity> soarActivity() {
    return BirdBrain.soar(
        ...
        new OneRandomBehaviour<>(
                Pair.of(new SetRandomFlightTarget<>(), 5),
                Pair.of(SetAdultWalkTarget.create(BirdUtils.STAY_NEAR_ENTITY_RANGE), 2))
            .startCondition(
                entity -> !entity.getBrain().hasMemoryValue(MemoryModuleType.WALK_TARGET)));
  }
