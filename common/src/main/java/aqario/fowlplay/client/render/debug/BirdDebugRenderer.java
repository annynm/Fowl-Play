...
import net.minecraft.client.renderer.RenderTypes; // Changed: RenderType -> RenderTypes (with 's')
...
public class BirdDebugRenderer implements FowlPlayDebugRenderers.LerpedDebugRenderer {
  ...
  private void updateTargetedEntity() {
    DebugRenderer.getTargetedEntity(this.client.getCameraEntity(), 16)
  ...
  private boolean isClose(BirdDebugPayload.BirdData birdData) {
    ...
    // noinspection ConstantConditions
    BlockPos playerPos =
        BlockPos.containing(playerEntity.getX(), birdData.pos().y(), playerEntity.getZ());
  ...
  private static void drawBirdData(
      PoseStack matrices,
      MultiBufferSource vertexConsumers,
      BirdDebugPayload.BirdData birdData,
      boolean targeted,
      double cameraX,
      double cameraY,
      double cameraZ,
    ...
    drawString(
        ...
        oldPos,
        i,
        "trusting: " + Arrays.toString(birdData.trusting().toArray()),
        -3355444,
        0.02F,
        partialTick);
    i++;
    ...
    drawString(
        ...
        oldPos,
        i,
        "move control: " + birdData.moveControl(),
        -1,
        0.02F,
        partialTick);
    i++;
    drawString(
        ...
        oldPos,
        i,
        "navigation: " + birdData.navigation(),
        -1,
        0.02F,
        partialTick);
    i++;
    ...
    drawString(
        ...
        i,
        "health: "
            + String.format(Locale.ROOT, "%.1f", birdData.health())
            + " / "
            + String.format(Locale.ROOT, "%.1f", birdData.maxHealth()),
        j,
        0.02F,
        partialTick);
    i++;
  ...
  public static void drawPath(
    ...
    PathfindingRenderer.renderPathLine(
        matrices,
        vertexConsumers.getBuffer(RenderTypes.debugLineStrip(6.0)),
        path,
        cameraX,
        cameraY,
        cameraZ);
  ...
  // REMOVED: drawString overload using BrainDebugRenderer.PoiInfo (class no longer exists or is
  // inaccessible in 1.21.11)
  // That overload was not used anywhere in this file (only the other drawString methods are
  // called).
  // If external code called it, replace with appropriate alternative, but it's safe to remove.
  ...
  public static void renderFloatingText(
    ...
    if (camera.isInitialized()) {
      ...
      //            poseStack.mulPose(Axis.YP.rotationDegrees((float) Math.toDegrees(Math.atan2(cx -
      // x, cz - z))));
